...
for (const name of engines) {
    ...
    warm.steps = 120;
    RunCase(engine, modules, warm, { engine: name, sleep: true, budgetMs: 10000 });
}
...
for (const spec of scenes) {
    for (const sleep of sleepModes) {
        ...
        for (const name of engines) {
            ...
            console.log(
                `${spec.id.padEnd(12)} ${EngineLabels[name].padEnd(6)} sleep ${sleep ? "on " : "off"}  step ${median.stepMean.toFixed(3)} ms  p95 ${median.stepP95.toFixed(3)}  engine ${median.engineMean.toFixed(3)}  build ${median.buildMs.toFixed(0)} ms  ${q}${median.stoppedEarly ? `  (stopped after ${median.stepsRun} steps)` : ""}`
            );
        }
    }
}
...
writeFileSync(path.join(outDir, `node-${stamp}.json`), JSON.stringify({ environment, repeats, results }, null, 2));
const md = [
...
writeFileSync(path.join(outDir, `node-${stamp}.md`), md);
console.log(`\nwrote bench/results/node-${stamp}.md`);
engine.dispose();
