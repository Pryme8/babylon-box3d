...
// Box3D physics showcase: npm run demo, then http://localhost:5178/?demo=pyramid
...
import { PhysicsShapeType, PhysicsMotionType, PhysicsConstraintAxis, PhysicsConstraintMotorType, PhysicsEventType } from "@babylonjs/core/Physics/v2/IPhysicsEnginePlugin";
import {
    PhysicsShapeBox,
    PhysicsShapeSphere,
...
import { HingeConstraint, BallAndSocketConstraint, DistanceConstraint, LockConstraint, PrismaticConstraint } from "@babylonjs/core/Physics/v2/physicsConstraint";
...
function Hud(ctx: IDemoContext, demo: Demo): void {
    ...
    const links = Demos.map((d) => (d === demo ? `<b>${d}</b>` : `<a style="color:#8cf" href="?demo=${d}">${d}</a>`)).join(" · ");
...
async function Main(): Promise<void> {
    ...
    raw.forEach((value, key) => searchParams.set(key.toLowerCase(), value));
...
Main().catch((e) => console.error("Box3D demo failed to start", e));
